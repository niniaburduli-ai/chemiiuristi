 ...
 * in themselves, same as any other missing field. City gets the identical
 * explicit no-default-guess rule (user-specified 2026-08-13, live testing
 * caught the model defaulting to "თბილისი"/Tbilisi as the capital) — city
 * must never be assumed just because most users are Tbilisi-based. */
function headerAddendum(locale: string): string {
  return locale === "en"
    ? `\n\nDocument header (mandatory, custom documents only):\n- Immediately after the title line, write one line with the city and date in this exact shape: "City of **[RED: city]**" then exactly 8 space characters (never more, never a tab, never a new line) then "**[RED: date]**" — unless the details clearly state a city and/or date, in which case use that value instead of the corresponding placeholder. Never guess, assume, or default the city to Tbilisi (or any other city) just because it's common — leave it as the placeholder unless the details explicitly name a city. Never guess or fill in today's date yourself — the date the document is actually filed may differ from the date it was drafted, so leave it for the user unless the details state one explicitly.`
    : `\n\nდოკუმენტის თავსართი (სავალდებულო, მხოლოდ თავისუფალი ფორმის დოკუმენტებისთვის):\n- სათაურის სტრიქონის ზუსტად შემდეგ დაწერე ერთი სტრიქონი ქალაქითა და თარიღით, ზუსტად ამ ფორმით: "ქ. **[წითელი: ქალაქი]**" შემდეგ ზუსტად 8 space სიმბოლო (არასოდეს მეტი, არასოდეს tab, არასოდეს ახალი ხაზი) და "**[წითელი: თარიღი]**" — გარდა იმ შემთხვევისა, როცა დეტალებში ცალსახად მითითებულია ქალაქი და/ან თარიღი, მაშინ ის მნიშვნელობა გამოიყენე შესაბამისი placeholder-ის ნაცვლად. არასოდეს გამოიგონო, არასოდეს ჩათვალო ან ავტომატურად ჩასვა „თბილისი" (ან სხვა რომელიმე ქალაქი) მხოლოდ იმიტომ, რომ ეს ყველაზე ხშირი შემთხვევაა — ქალაქი დატოვე placeholder-ად, თუ დეტალებში ცალსახად მითითებული არ არის. არასოდეს გამოიგონო ან თავად ჩასვა დღევანდელი თარიღი — დოკუმენტის შედგენის თარიღი და ფაქტობრივი შეტანის/გაგზავნის თარიღი შეიძლება განსხვავდებოდეს, ამიტომ თარიღი დატოვე მომხმარებლისთვის შესავსებად, თუ დეტალებში ცალსახად მითითებული არ არის.`;
}
